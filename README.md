...
下面给出一个最小可用的并发 HTTP 服务方案，适合把仓库中的 ONNX 推理封装成在线服务。
...
新增入口文件为 `service.py`。它会在进程启动时只加载一次模型，然后通过线程池承接阻塞推理，并用异步信号量限制并发数。
...
如果未传 `--onnx-path`，服务会自动下载并使用 `TurnSense1.1` 的 `INT8` 模型到 `models/model_int8.onnx`，并下载 `models/am.mvn`。

参数含义：

- `--max-concurrency`：服务层允许同时排队执行的最大推理请求数。
- `--max-workers`：实际执行阻塞推理的线程池大小。
- `--use-cuda`：如果机器上 `onnxruntime-gpu` 可用，可开启 CUDA provider。
...
curl -X POST http://127.0.0.1:8000/infer/json \
  ...
  -d '{"audio_base64":"<base64-audio>","source":"demo.wav"}'
```
...
curl -X POST http://127.0.0.1:8000/infer/bytes \
  ...
  -H "X-Audio-Source: demo.wav" \
  --data-binary @examples/complete.wav
```

如果发送的是裸 PCM 字节流，请额外带上 PCM 格式信息。下面示例为 `16kHz`、单声道、`s16le`：

```bash
curl -X POST http://127.0.0.1:8000/infer/bytes \
  -H "Content-Type: application/octet-stream" \
  -H "X-Audio-Source: demo.pcm" \
  -H "X-Audio-Format: pcm_s16le" \
  -H "X-Audio-Sample-Rate: 16000" \
  -H "X-Audio-Channels: 1" \
  --data-binary @examples/demo.pcm
```

也可使用 `Content-Type: audio/pcm`，此时默认按 `pcm_s16le` 解析；支持的 PCM 格式包括 `pcm_s16le`、`pcm_s16be`、`pcm_s32le`、`pcm_f32le`、`pcm_u8`。

### 4. 并发设计说明

当前仓库的推理是阻塞式 ONNX 推理，因此服务层采用的是“异步接入 + 线程池执行”的方式：

- HTTP 使用 `FastAPI` 的异步接口，请求处理不会互相阻塞。
- 模型只初始化一次，避免每个请求重复加载 ONNX。
- 通过 `asyncio.Semaphore` 控制最大并发，避免机器被瞬时流量打满。
- 通过 `ThreadPoolExecutor` 承接 `onnxruntime` 和特征提取这类阻塞计算。

如果你的目标是更高吞吐：

- 单机 CPU：优先调 `--max-workers` 和 `--max-concurrency`。
- 单机 GPU：建议先保持单进程单模型，再压测是否要扩到多 worker。
- 多实例部署：建议在 Nginx 或网关后横向扩容多个 `service.py` 进程。
...
如果你直接运行 `python service.py` 且未传 `--onnx-path`，服务会自动下载 `TurnSense1.1` 的 `INT8` 模型和 `am.mvn` 到 `models/` 目录。
